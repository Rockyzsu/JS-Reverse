...
function encrypt(string, encryptionExponent, modulus, maxdigits) {
...
while (encrypt.length % this.chunkSize !== 0) {
    encrypt[j++] = 0;
}

//var encryptObject = encrypt;
var charCounter = 0;
var j, block;
var encrypted = "";
...
block = new BigInt();
j = 0;
for (var k = charCounter; k < charCounter + this.chunkSize; ++j) {
    block.digits[j] = encrypt[k++];
    block.digits[j] += encrypt[k++] << 8;
}
var crypt = this.barrett.powMod(block, this.e);
var text = this.radix == 16 ? biToHex(crypt) : biToString(crypt, keyPair.radix);
encrypted += text + " ";
charCounter += this.chunkSize;
